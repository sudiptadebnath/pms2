...
$(document).ready(function () {
    ...
    <?php if($rowreorder): ?>
        rowReorder: {
            dataSrc: '<?php echo e($rowreorder[0]); ?>'
        },
    <?php endif; ?>
    <?php if($responsive): ?>
        responsive: true,
    ...
    <?php if($rowreorder): ?>
    <?php echo e($name); ?>.on('row-reorder', function (e, diff, edit) {
        let order = [];
        for (let i = 0; i < diff.length; i++) {
            order.push({
                id: <?php echo e($name); ?>.row(diff[i].node).data().id,
                position: diff[i].newPosition 
            });
        }
        $.ajax({
            url: '<?php echo e($rowreorder[1]); ?>',
            method: 'POST',
...
<?php $__env->stopPush(); ?>
<?php /**PATH C:\xampp\htdocs\pms2\resources\views/components/table.blade.php ENDPATH**/ ?>